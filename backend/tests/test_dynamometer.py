...
def test_all_seven_classes_generate_valid_closed_loops():
    ...
    for label in VALID_LABELS:
        ...
        assert len(card) == 101, f"Expected 101 points (100 + closing point) for {label}"
...
def test_dynamometer_classes_are_statistically_separable():
    ...
    for label in VALID_LABELS:
        ...
        metrics[label] = {
            ...
            "mean_downstroke_var": sum(downstroke_variances) / len(downstroke_variances),
        }
    ...
    area_ratio = metrics["normal"]["mean_area"] / max(metrics["parted_rod"]["mean_area"], 1.0)
    assert area_ratio > 15.0, "Normal card must have >15x the enclosed area of parted rod"
    ...
    assert metrics["rod_float"]["mean_min_load"] < metrics["normal"]["mean_min_load"] - 2000.0

    # 4. Fluid pound has significantly higher downstroke variance than gas interference
    # due to the abrupt impact shockwave
    assert metrics["fluid_pound"]["mean_downstroke_var"] > metrics["gas_interference"]["mean_downstroke_var"]
