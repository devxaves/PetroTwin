...
REQUEST_COUNT = Counter(
    "thermotwin_http_requests_total",
    "Total HTTP requests processed by ThermoTwin API",
    ["method", "endpoint", "status"],
)

RECOMMENDATIONS_GENERATED = Counter(
    "thermotwin_recommendations_generated_total",
    "Total coupled CSS+SRP digital twin recommendations generated",
    ["well_id"],
)

APPROVALS_RECORDED = Counter(
    "thermotwin_operator_approvals_total",
    "Total operator approvals logged in audit trail",
    ["decision"],
)

REQUEST_LATENCY = Histogram(
    "thermotwin_request_duration_seconds",
